...
var lightbox_path = lightbox_path || '/images/';
(function($) {
	/**
	...
	$.fn.lightBox = function(settings) {
		...
		settings = jQuery.extend({
			...
			imageLoading:			lightbox_path+'jquery.lightbox/lightbox-ico-loading.gif',		// (string) Path and the name of the loading icon
			imageBtnPrev:			lightbox_path+'jquery.lightbox/lightbox-btn-prev.gif',			// (string) Path and the name of the prev button image
			imageBtnNext:			lightbox_path+'jquery.lightbox/lightbox-btn-next.gif',			// (string) Path and the name of the next button image
			imageBtnClose:			lightbox_path+'jquery.lightbox/lightbox-btn-close.gif',		// (string) Path and the name of the close btn
			imageBlank:				lightbox_path+'jquery.lightbox/lightbox-blank.gif',			// (string) Path and the name of a blank image (one pixel)
			// Configuration related to container image box
			containerBorderSize:	10,			// (integer) If you adjust the padding in the CSS for the container, #lightbox-container-image-box, you will need to update this value
			containerResizeSpeed:	320,		// (integer) Specify the resize duration of container image. These number are miliseconds. 400 is default.
			...
			,shrinkPadding:			12			// (integer) The padding amount for the shrinking calculation *** FULLER
		...
		function _start(objClicked,jQueryMatchedObj) {
			...
			if ( jQueryMatchedObj.length == 1 ) {
				...
				settings.imageArray.push(new Array(objClicked.getAttribute('href') || objClicked.getAttribute('src'),position));
			} else {
				// Add an Array (as many as we have), with href and title atributes, inside the Array that storage the images references
				for ( var i = 0; i < jQueryMatchedObj.length; i++ ) {
					if( settings.captionPosition != 'gallery' ) {
						var position = jQueryMatchedObj[i].getAttribute('title');
						settings.imageArray.push(new Array(jQueryMatchedObj[i].getAttribute('href') || jQueryMatchedObj[i].getAttribute('src'),position));
					} else {
						if( i > 1 ) {
							var href1 = jQueryMatchedObj[i-1].getAttribute('href') || jQueryMatchedObj[i-1].getAttribute('src');
							var href2 = jQueryMatchedObj[i].getAttribute('href') || jQueryMatchedObj[i].getAttribute('src');
							var href3 = jQuery(jQueryMatchedObj[i-1]).parent().parent().children().children().attr('href') || jQuery(jQueryMatchedObj[i-1]).parent().parent().children().children().attr('src');
							var href4 = jQuery(jQueryMatchedObj[i]).parent().parent().children().children().attr('href') || jQuery(jQueryMatchedObj[i]).parent().parent().children().children().attr('src');
							if( !(href1 != href2 && href3 == href4)) {
								var position = jQuery(jQueryMatchedObj[i]).parent().next().html();
								settings.imageArray.push(new Array(href4,position));
							}
						} else {
								var position = jQuery(jQueryMatchedObj[i]).parent().next().html();
								var href1 = jQuery(jQueryMatchedObj[i]).parent().parent().children().children().attr('href') || jQuery(jQueryMatchedObj[i]).parent().parent().children().children().attr('src');
								settings.imageArray.push(new Array(href1,position));
						}
					}
				}
			}
			while ( settings.imageArray[settings.activeImage][0] != (objClicked.getAttribute('href') || objClicked.getAttribute('src')) ) {
